...
!macro customInstall
  ...
  ${Else}
    ...
    ${If} $LANGUAGE == 1042
      ...
      FileWrite $0 "- 진단 결과: 충돌하거나 선점 중인 타사 기본 프로그램이 탐색되지 않음 (공석)$\r$\n"
      FileWrite $0 "- 조치 사항: 사용자의 기존 환경을 훼손하지 않는 선에서 압축파일만 자동 연결 완료!$\r$\n"
    ${Else}
      ...
      FileWrite $0 "- Result: No conflicting third-party default program was found (Empty slot).$\r$\n"
      FileWrite $0 "- Action: Successfully auto-associated archive extensions without disrupting your current setup!$\r$\n"
    ${EndIf}
    FileClose $0
